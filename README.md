...
> 当前版本为 beta。默认检索近五年英文文献，每个方向选择 1 篇，每两天约 07:30（`Asia/Shanghai`）运行。

## 主要能力

- 通过 `LITERATURE_DIRECTIONS_JSON` 配置 1–5 个研究方向，无需修改源码。
- 主动检索 OpenAlex Works，而不是依赖邮件是否到达。
- 过滤撤稿、非英文、超出时间范围、非论文类型及已推送候选。
...
| `LITERATURE_DIRECTIONS_JSON` | 否 | 自定义研究方向；不填则使用内置示例方向 |
...
将 JSON 数组保存为 `LITERATURE_DIRECTIONS_JSON`。例如：
...
完整双方向示例见 [`examples/directions.example.json`](examples/directions.example.json)。
...
最多配置 5 个方向，单个关键词最多 100 字符。关键词会去除首尾空白并按大小写不敏感方式去重。Apps Script 对单个属性值另有 [9 KB 平台配额](https://developers.google.com/apps-script/guides/services/quotas)，配置应保持精简。显式配置无效时程序会停止并给出字段错误，不会悄悄回退到默认主题。
...
node --check < Code.gs
node -e "const fs=require('fs'); ['appsscript.json','examples/directions.example.json'].forEach(f=>JSON.parse(fs.readFileSync(f,'utf8'))); console.log('json ok')"
```
...
确认属性名完全等于 `LITERATURE_DIRECTIONS_JSON`，保存后运行 `validateLiteratureRadarConfig()`。属性缺失或仅包含空白时才会使用默认方向。

### 主动检索没有候选

先运行 `testEveryTwoDaysDryRun()`。常见原因是 API Key 无效、关键词过窄、所有候选已推送，或文献不满足近五年/英文/非撤稿条件。
